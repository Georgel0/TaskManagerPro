...
export function getSnapRect(pattern, n) {
  ...
  switch (pattern) {
    case 'grid': {
      ...
      return { x: WORKSPACE_X + col * cW + PAD, y: row * cH + PAD, w: cW - PAD * 2, h: cH - PAD * 2 };
    }

    case 'master': {
      const masterW = Math.floor(availW * 0.58);
      if (n === 0) return { x: WORKSPACE_X + PAD, y: PAD, w: masterW - PAD * 2, h: availH - PAD * 2 };
      const slot = (n - 1) % 3;
      const slotH = Math.floor(availH / Math.min(n, 3));
      return { x: WORKSPACE_X + masterW + PAD, y: slot * slotH + PAD, w: availW - masterW - PAD * 2, h: slotH - PAD * 2 };
    }

    case 'columns': {
      const col = n % 3;
      const colW = Math.floor(availW / 3);
      return { x: WORKSPACE_X + col * colW + PAD, y: PAD, w: colW - PAD * 2, h: availH - PAD * 2 };
    }

    case 'rows': {
      const row = n % 3;
      const rowH = Math.floor(availH / 3);
      return { x: WORKSPACE_X + PAD, y: row * rowH + PAD, w: availW - PAD * 2, h: rowH - PAD * 2 };
    }
...
export const cascadePos = (type) => {
  ...
  const off = (_ci++ % 8) * 28;
  if (typeof window === 'undefined') return { x: WORKSPACE_X + 60 + off, y: 40 + off };
